...
export const LudoBoard: React.FC<{ socket: Socket | null }> = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('ludo-update'); };
  }, [socket]);

  if (!game) return <div className="text-center font-bold p-8">Loading Arena...</div>;
  ...
  if (game.status === 'setup') {
    return (
      <div className="w-full h-full flex items-center justify-center bg-[#153461] p-4">
        <div className="bg-white border-4 border-dark rounded-xl p-8 text-center max-w-sm w-full shadow-2xl">
          <h2 className="text-2xl font-black mb-6">Select Players</h2>
  ...
  const HUDPanel = ({ color, isReversed }: { color: PlayerColor, isReversed?: boolean }) => {
    ...
    // You can interact if it's your turn AND (it's your color OR it's unclaimed and you don't own another color yet)
    ...
    return (
      <div className={`flex flex-col gap-1 ${!isReversed && 'items-end'}`}>
        ...
        <span className={`text-[10px] sm:text-xs font-bold text-white uppercase bg-dark/50 px-2 py-0.5 rounded ${isMyColor ? 'text-green-300' : ''}`}>
          {displayName}
        </span>
      </div>
    );
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-[#102a5c] to-[#0a1930] relative overflow-hidden">
      ...
      <button onClick={resetGame} className="absolute top-2 right-2 text-white/50 hover:text-white text-xs underline z-50">End Game</button>
        ...
        <div className="relative w-full aspect-square bg-white border-4 border-white shadow-2xl">
          ...
          {(['red', 'green', 'yellow', 'blue'] as const).map((color) => 
            game.positions[color].map((pos, idx) => {
              ...
              // Only let you click the piece if you own the color
